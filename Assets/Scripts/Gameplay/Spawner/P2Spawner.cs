...
public class P2Spawner : MonoBehaviour {
	...
	{
		GameObject unit = PhotonNetwork.Instantiate (Path.Combine ("Prefabs/GameUnit/P2Enemies", enemy1Prefab.name), spawnPoint.position, spawnPoint.rotation, 0);
	...
	{
		GameObject unit = PhotonNetwork.Instantiate (Path.Combine ("Prefabs/GameUnit/P2Enemies", enemy2Prefab.name), spawnPoint.position, spawnPoint.rotation, 0);
		Enemy objScript = unit.GetComponent<Enemy> ();
		objScript.SetStat (a, b, c);
	}
	...
	{
		GameObject unit = PhotonNetwork.Instantiate (Path.Combine ("Prefabs/GameUnit/P2Enemies", enemy3Prefab.name), spawnPoint.position, spawnPoint.rotation, 0);
		Enemy objScript = unit.GetComponent<Enemy> ();
		objScript.SetStat (a, b, c);
	}
